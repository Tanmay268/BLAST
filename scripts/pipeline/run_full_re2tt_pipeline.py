...
# ======================================================
# BLAST — FULL RE2-TT (TRAIN TICKET) BATCH PIPELINE
# ======================================================
#
# The cross-system expansion (ADR-019's recommended next
# step). Directly modeled on run_full_re2ob_pipeline.py --
# same resumable download -> distill -> delete-raw pattern,
# same shared blast_journey_lib.py logic (verified against
# Online Boutique; no drift). Only the target service list
# and output filenames differ.
#
# 90 cases = 5 target services x 6 fault types x 3
# repetitions, confirmed by direct HF enumeration (not
# assumed) before this script was written: ts-auth-service,
# ts-order-service, ts-route-service, ts-train-service,
# ts-travel-service.
#
# Unlike Online Boutique, ALL 90 Train Ticket cases have
# traces.parquet (verified) -- Sock Shop (RE2-SS) does not
# (0/90 cases), which is why only Train Ticket is being
# built here.
# ======================================================
...
BASE_DIR = Path("./data")
CHECKPOINT_DIR = Path("./results/data/re2tt_journey_checkpoints")
SIGNATURE_CHECKPOINT_DIR = Path("./results/data/re2tt_journey_signature_checkpoints")

EXCLUDED_CASES_FILE = "results/data/re2tt_excluded_cases.csv"
FINAL_OUTPUT = "results/data/re2tt_journey_impairment_full.csv"
FINAL_SIGNATURE_CATALOG = "results/data/re2tt_journey_signature_catalog_full.csv"

CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
SIGNATURE_CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for svc in TARGET_SERVICES:
        for fault in FAULT_TYPES:
            for inst in INSTANCES:
                cases.append((f"re2tt_{svc}_{fault}_{inst}", svc, fault))
    return cases
...
def main():
    ...
    print("=" * 110)
    print("BLAST — FULL RE2-TT (TRAIN TICKET) BATCH PIPELINE")
    print("=" * 110)
        ...
        try:
            ...
            if short_window:
                excluded_rows.append({
                    "case": case, "target_service": svc, "fault_type": fault_type,
                    "reason": "short_window",
            ...
            n_types = len(summary)
            print(f"  OK -- {n_types} journey types, {n_impaired} impaired")

            if summary["signature"].fillna("").eq("").all() or (summary["signature"].fillna("") == "").all():
                print(f"  WARNING: every journey type for {case} has an EMPTY signature -- "
                      f"capability attribution will find nothing to map. Check the journey-"
                      f"typing rule against this case's trace schema before trusting results.")
            ...
            excluded_rows.append({
                "case": case, "target_service": svc, "fault_type": fault_type,
                "reason": type(e).__name__, "detail": str(e)[:300], "hard_exclusion": True,
            })
    ...
    all_summaries = pd.concat([pd.read_csv(f) for f in checkpoint_files], ignore_index=True)
    all_summaries.to_csv(FINAL_OUTPUT, index=False)

    sig_files = sorted(SIGNATURE_CHECKPOINT_DIR.glob("*.csv"))
    if sig_files:
        all_signatures = pd.concat([pd.read_csv(f) for f in sig_files], ignore_index=True)
        catalog = (
            all_signatures
    ...
    if remaining > 0:
        print(f"\n{remaining} cases still unprocessed (hard failures). Re-run to retry.")
    else:
